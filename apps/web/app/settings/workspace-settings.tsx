...
export function WorkspaceSettings() {
  ...
  async function load() {
    const [membersResponse, keysResponse, webhooksResponse] = await Promise.all([
    ...
    if (!membersResponse.ok || !keysResponse.ok || !webhooksResponse.ok) {
  ...
  return (
    <div className="settings-grid">
      ...
      <section className="panel settings-card">
        ...
        <div className="settings-form"><input onChange={(event) => setName(event.target.value)} placeholder="Display name" value={name} /><input onChange={(event) => setActorId(event.target.value)} placeholder="Actor or identity ID" value={actorId} /><select onChange={(event) => setRole(event.target.value)} value={role}><option value="viewer">Viewer</option><option value="coordinator">Coordinator</option><option value="producer">Producer</option><option value="legal_reviewer">Legal reviewer</option><option value="post_supervisor">Post supervisor</option></select><button className="secondary-button" disabled={!name || !actorId} onClick={() => void addMember()} type="button">Add member</button></div>
      </section>
      <section className="panel settings-card">
