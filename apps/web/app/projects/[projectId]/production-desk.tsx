...
type DocumentVersion = { id: string; original_filename: string; version_number: number; parent_document_id: string | null; status: string; created_at: string };
  ...
  return (
    <section className="panel production-desk">
      ...
      <div className="desk-grid">
        <div className="desk-section"><div className="operations-section-heading"><span>Screenplay versions</span><small>{documents.length} versions</small></div>{documents.map((document) => <div className="desk-row" key={document.id}><div><strong>v{document.version_number} · {document.original_filename}</strong><small>{document.status} · {dateLabel(document.created_at)}</small></div><span className="table-status approved">{document.parent_document_id ? "Revision" : "Original"}</span></div>)}{documents.length > 1 ? <button className="secondary-button desk-action" disabled={busy} onClick={() => void compareVersions()} type="button">Compare latest versions</button> : <p className="operations-empty">Upload a new screenplay version to see a source diff.</p>}{diff ? <div className="diff-callout"><strong>Latest version diff</strong><span>{diff.added_lines} added · {diff.removed_lines} removed · {diff.changed_lines} changed lines</span>{diff.added_assets.length ? <small>New asset signals: {diff.added_assets.join(", ")}</small> : null}{diff.removed_assets.length ? <small>Removed asset signals: {diff.removed_assets.join(", ")}</small> : null}</div> : null}</div>
