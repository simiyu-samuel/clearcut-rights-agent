  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="nav"><Link className="nav-item" href="/"><span className="nav-icon">⌂</span>Projects</Link><Link className="nav-item active" href={"/reports" as Route}><span className="nav-icon">↗</span>Reports</Link></nav>
      ...
      <main className="main">
        ...
        <div className="content"><section className="hero"><div><div className="eyebrow">Clearance reporting</div><h1>Project reports.</h1><p>Export an evidence-backed clearance summary for any project in the workspace.</p></div><Link className="primary-button" href="/projects/new">+ New project</Link></section><section className="panel report-list">{projects.length === 0 ? <div className="review-empty">No projects yet. Create a project to generate a report.</div> : projects.map((project) => <div className="report-row" key={project.id}><div><strong>{project.title}</strong><span>{project.project_type} · {project.status}</span></div><ReportButton projectId={project.id} /></div>)}</section></div>
      </main>
    </div>
  );
}
