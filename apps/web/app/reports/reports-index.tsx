...
export function ReportsIndex() {
  ...
  return (
    <WorkspaceShell active="reports" breadcrumbs={<><Link href="/">Projects</Link><span>/</span><strong>Reports</strong></>}>
      ...
      {preview ? <div className="drawer-layer" onClick={() => setPreview(null)}><aside aria-label="Report preview" className="report-preview-drawer" onClick={(event) => event.stopPropagation()}><div className="drawer-header"><div><span className="eyebrow">Report snapshot</span><h2>{preview.project.title}</h2></div><button aria-label="Close report preview" className="icon-button" onClick={() => setPreview(null)} type="button">×</button></div><div className="report-preview-cover"><span>ClearCut</span><strong>Evidence-backed clearance report</strong><small>Version {preview.reports[0]?.version_number} · {preview.reports[0] ? new Intl.DateTimeFormat("en-GB", { dateStyle: "medium" }).format(new Date(preview.reports[0].created_at)) : ""}</small></div><div className="report-preview-details"><div><span>Project type</span><strong>{preview.project.project_type}</strong></div><div><span>Territories</span><strong>{preview.project.territories.join(" · ") || "Not set"}</strong></div><div><span>Distribution</span><strong>{preview.project.distribution_modes.join(" · ") || "Not set"}</strong></div><div><span>Record</span><strong>Human review required</strong></div></div><p className="report-preview-note">Open the full report to inspect asset-level evidence, decisions, permission work, and the styled PDF export.</p><div className="report-preview-actions"><Link className="primary-button" href={`/reports/${preview.project.id}` as Route}>Open full report</Link><button className="secondary-button" onClick={() => setPreview(null)} type="button">Close preview</button></div></aside></div> : null}
    </WorkspaceShell>
  );
}
