...
import { authErrorMessage, demoLoginIsConfigured } from "@/lib/auth";
...
function AuthEditorial() {
  return <aside className="auth-editorial"><div className="auth-editorial-brand"><img className="auth-editorial-logo" src="/clearcut-logo.png" alt="ClearCut" /></div><div className="auth-editorial-main"><span className="eyebrow">Production-office editorial</span><h2>Turn creative material into an evidence-backed rights plan.</h2><p>Keep every source, research finding, human decision, and delivery record connected to the production.</p></div><div className="auth-editorial-list"><div><span className="auth-feature-mark">01</span><div><strong>Evidence-backed research</strong><small>Traceable sources for every rights signal.</small></div></div><div><span className="auth-feature-mark">02</span><div><strong>Human review gates</strong><small>Clear decisions and accountability across the team.</small></div></div><div><span className="auth-feature-mark">03</span><div><strong>Delivery-ready reports</strong><small>Export a production record when the work is complete.</small></div></div></div><small className="auth-editorial-footer">© 2026 ClearCut AI · Rights operations workspace</small></aside>;
}
...
export function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  if (auth.status === "loading") {
    return <main className="auth-screen auth-layout"><AuthEditorial /><div className="auth-stage"><Brand /><div className="auth-card panel"><span className="eyebrow">Secure workspace</span><h1>Loading your workspace…</h1><p>Checking your ClearCut identity and access.</p></div></div></main>;
  }
  if (auth.status === "configuration_error") {
    return <main className="auth-screen auth-layout"><AuthEditorial /><div className="auth-stage"><Brand /><div className="auth-card panel"><span className="eyebrow">Workspace configuration</span><h1>Authentication needs setup.</h1><p>Set the Firebase client configuration and API audience before using Identity Platform mode.</p></div></div></main>;
  }
  if (auth.status === "workspace_error") {
    return <main className="auth-screen auth-layout"><AuthEditorial /><div className="auth-stage"><Brand /><div className="auth-card panel"><span className="eyebrow">Workspace access</span><h1>We could not load your workspace.</h1><p>{auth.errorMessage ?? "Refresh and try again."}</p><button className="primary-button" onClick={() => window.location.reload()} type="button">Refresh workspace</button></div></div></main>;
  }
  if (auth.status === "signed_out") {
    ...
    async function signInDemo() {
      ...
      setResetSent(false);
      setAuthBusy(true);
      try { await auth.signInWithDemo(); } catch (error) { setSignInMessage(authErrorMessage(error)); }
